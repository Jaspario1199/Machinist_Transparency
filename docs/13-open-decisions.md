...
| D-28 | Who may sign in at a shared wall terminal, and does an idle terminal auto-lock? | **Open** | IT/OT owner + production supervisor | Phase 4 permissions | The demo makes sign-in explicit and audits handover, and refuses every action while nobody is signed in. It has no idle timeout — on a real floor a terminal left signed in attributes the next person's decisions to whoever walked away |
| D-29 | Does a first-article approval need a second person, or may the machinist release their own setup? | **Open** | Quality | Phase 4 workflow | The demo lets the machinist release it and records who did. Quality may require an independent release for some part classes |
| D-30 | Is cycle history pooled across machines running the same program, or kept per machine? | **Open** | Manufacturing engineering | ETA accuracy | The demo keeps it per machine — same program, different spindle and fixture is a different distribution. Pooling would narrow cold-start ranges at the cost of accuracy on machines that differ |
| D-31 | Is the machine-data collection layer built internally or bought from an MDC vendor? | **Open** | Technical owner + sponsor | `BOM-021` scope, Phase 3 | 500 internal hours at the BOM's own $30–80/hr is $15k–$40k, more than the entire hardware budget, and collection is the part vendors already sell well. No vendor sells the queue-governance layer this project exists for. A hybrid — buy collection, build governance — keeps internal effort on the part nobody sells and takes the most machine-specific engineering off the critical path. See `docs/06` |
