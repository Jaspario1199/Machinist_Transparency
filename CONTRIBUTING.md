...
### Running the checks automatically

```bash
bash scripts/install-hooks.sh
```

This installs a pre-push hook that regenerates the derived files, fails if they
were stale, runs the secret scan and runs the test suite — the same gates as CI,
about thirty seconds. Opt-in, because it is your push it slows down. Remove it
with `rm .git/hooks/pre-push`; skip it once with `git push --no-verify`.

Worth installing regardless of CI, and necessary while CI cannot allocate a
runner — otherwise nothing is checking these before code reaches the branch.
